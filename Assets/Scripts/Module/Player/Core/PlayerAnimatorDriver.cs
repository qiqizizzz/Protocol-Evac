...
namespace Module.Player.Core
{
    public sealed class PlayerAnimatorDriver
    {
            ...
            PlayerMoveConfigSO moveConfig)
        {
            ...
            {
                QLog.Error("初始化玩家动画驱动失败：PlayerStateMachine 为空");
                return;
            }

            if (moveConfig == null)
            {
                QLog.Error("初始化玩家动画驱动失败：PlayerMoveConfigSO 为空");
                return;
            }
        ...
        // 根据状态机裁决结果同步 Animator 参数
        public void Tick(float deltaTime)
        {
            if (!m_isInited)
                return;

            bool isMoving = m_stateMachine.CurrentLeafStateId == PlayerStateId.GroundedMove;
            bool isSprinting = isMoving && m_context.TargetMoveSpeed > m_moveConfig.WalkSpeed;
